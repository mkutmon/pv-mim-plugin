...
import gov.nih.nci.lmp.mim.mimVisLevel1.AnchorGlyphType;
import gov.nih.nci.lmp.mim.mimVisLevel1.ArrowHeadEnumType;
import gov.nih.nci.lmp.mim.mimVisLevel1.DiagramDocument;
import gov.nih.nci.lmp.mim.mimVisLevel1.DiagramType;
import gov.nih.nci.lmp.mim.mimVisLevel1.EntityControlledVocabularyType;
import gov.nih.nci.lmp.mim.mimVisLevel1.EntityGlyphType;
import gov.nih.nci.lmp.mim.mimVisLevel1.GroupEnumType;
import gov.nih.nci.lmp.mim.mimVisLevel1.GroupType;
import gov.nih.nci.lmp.mim.mimVisLevel1.InteractionGlyphType;
import gov.nih.nci.lmp.mim.mimVisLevel1.MimBioType;
import gov.nih.nci.lmp.mim.mimVisLevel1.PublicationXRefType;
import gov.nih.nci.lmp.mim.mimVisLevel1.RelationshipXRefType;
...
public class ExporterHelper extends CommonHelper {
	...
	private void mapBiopax(PathwayElement pwElem) {
		for(BiopaxNode node : pwElem.getBiopaxReferenceManager().getReferences()) {
			...
			mimPubXRef.setTitle(xref.getTitle());

			mimPubXRef.setYear(xref.getYear());
			mimPubXRef.setId(xref.getPubmedId());

			// TODO: Remove assumption that publication DB is always "PubMed"
			mimPubXRef.setDb("PubMed");

			mimPubXRef.setJournal(xref.getSource());
		}
	}
	...
	private ArrayList<String> mapBiopaxRefs(PathwayElement pwElem) {
		...
		for (PublicationXref gpmlPubXRef : pwElem.getBiopaxReferenceManager().getPublicationXRefs()) {
